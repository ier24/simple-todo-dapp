...
const useContent = (contract: ethers.Contract) => {
  ...
  useEffect(() => {
    const getTasks = async () => {
      const _taskCount = await taskCount(); // contract call.
...
export const App: VFC = () => {
  ...
  const signer = provider.getSigner(); // ローカルネットワークの１件目のアカウント取得（テストなどで使う）。
  const contract = new ethers.Contract(contractAddress, artifact.abi, provider);
  const contractWithSigner = contract.connect(signer); // コントラクトとアカウントを接続
  return (
    <div>
      <h1>hello, todo list contract</h1>
